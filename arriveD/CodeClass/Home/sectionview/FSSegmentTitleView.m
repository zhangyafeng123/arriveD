...
- (void)layoutSubviews
{
    ...
    for (NSString *title in self.titlesArr) {
        CGFloat itemBtnWidth = [FSSegmentTitleView getWidthWithString:title font:H14] + self.itemMargin;
    ...
    if (totalBtnWidth <= CGRectGetWidth(self.bounds)) {//不能滑动
        ...
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [FSSegmentTitleView getWidthWithString:self.titlesArr[idx] font:H14] + self.itemMargin;
            CGFloat itemBtnHeight = CGRectGetHeight(self.bounds);
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [FSSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:H14];
